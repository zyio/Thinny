...
You can add social links by add the following code inside the variable `social`:
...
You can add menu items by add the following code inside the variable `menu`:
...
- `video:`<i>`true`</i> `or `<i>`false`</i>: a cover video for the post (default value is _false_).
